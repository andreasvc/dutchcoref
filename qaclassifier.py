	...
	def add(self, trees, embeddings, quotations, gqm_dict, mentions, idx):
		...
		for q in quotations:
			...
			for m in self.candidates[cand_startcount:]:
				...
				# 	previousmentions = [mn for mn in mentions
				# 			if previousquote.start <= idx[mn.sentno, mn.begin]
				# 			and previousquote.end >= idx[mn.sentno, mn.begin]
				# 				+ (mn.end - mn.begin)]
...
def get_predictions(a, b, quote, candidates, probs, gold, pred,
...
def evaluate(validationfiles, parsesdir, annotationsdir, tokenizer, bertmodel):
	...
	for a, b, quote in quotedata:
		...
		get_predictions(a, b, quote, candidates, probs, gold, pred,
				closest=False, verbose=VERBOSE)
	...
	print(metrics.classification_report(
			y_val, pairpred, digits=3, zero_division=0))
...
def predict(trees, embeddings, quotations, mentions, idx, verbose=False):
	debug(color('speaker identification (neural)', 'yellow'))
	data = QuoteFeatures()
	gqm_dict = dict()
	data.add(trees, embeddings, quotations, gqm_dict, mentions, idx)
	if not data.result:
	...
	for a, b, quote in quotedata:
		if a == b:
			...
			m = (candidates[best] if probs[best] > QUOTE_PAIR_THRESHOLD
					else None)
			mo_predictions.append(m)
		if verbose:
			debug('attributing %s' % color(quote.text, 'green'))
		for n in range(a, b if verbose else a):
			debug('\t%d %d %s %s p=%.3f%s' % (
					candidates[n].sentno, candidates[n].begin,
					candidates[n].node.get('rel'), candidates[n],
					probs[n],
					' %s %g %s' % (
						'<>'[int(probs[best] > QUOTE_PAIR_THRESHOLD)],
						QUOTE_PAIR_THRESHOLD,
						color('best', 'green'))
						if n == best else ''))

	assert len(quotations) == len(mo_predictions)
	for quote, prediction in zip(quotations, mo_predictions):
		debug('attributed %s\n\tto %s with neural classifier' % (
				color(quote.text, 'green'), prediction))
...
def main():
	...
	trainfiles, validationfiles, parsesdir, annotationsdir = (
			args.train, args.val, args.parses, args.annotations)
	tokenizer, bertmodel = bert.loadmodel()

	# Train and evaluate
	if not args.eval:
		train(trainfiles, validationfiles, parsesdir, annotationsdir,
				tokenizer, bertmodel)
